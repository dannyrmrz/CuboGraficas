...
@vertex
fn vs_main(@location(0) pos: vec4<f32>, @location(1) normal: vec4<f32>, @location(2) tex_coords: vec2<f32>) -> Output {
    var output: Output;
...
@binding(3) @group(0) var texture_sampler: texture_2d<f32>;
@binding(4) @group(0) var texture_sampler_sampler: sampler;

@fragment
fn fs_main(@location(0) v_position: vec4<f32>, @location(1) v_normal: vec4<f32>, @location(2) v_tex_coords: vec2<f32>) ->  @location(0) vec4<f32> {
    ...
    // Sample texture
    let texture_color = textureSample(texture_sampler, texture_sampler_sampler, v_tex_coords);
    
    // Apply lighting to texture
    let lit_color = texture_color * (ambient + diffuse) + light_uniforms.specular_color * specular;
    return lit_color;
}
